...
class UserManager:
    ...
    def update_rodnoy_bonus(user_id, amount):
        ...
        cursor.execute("UPDATE users SET last_rodnoy_bonus_date = ?, daily_bonus_count = daily_bonus_count + 1, balance = balance + ? WHERE user_id = ?",
                      (today.strftime("%Y-%m-%d"), amount, user_id))
            ...
            # Диапазонду текшерүү
                ...
                if len(range_parts) == 2:
                    try:
                        ...
                        if bet_per_number < MIN_BET:
                            await update.effective_chat.send_message(f"❌ Слишком маленькая ставка на каждый номер! Минимум {MIN_BET} на номер.")
                            return
                        ...
                        for num in range(start_num, end_num + 1):
                            if num == 0:
                                success = await Games.handle_roulette_bet(update, context, "number", "0", bet_per_number)
                                bets_made.append(f"{bet_per_number} на 0")
                            else:
